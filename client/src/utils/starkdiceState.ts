...
const transformDiceRoll = (rawData: any) => {
    const data = rawData['dojo_starter-DiceRoll'];
    if (!data) return null;
...
const transformGame = (rawData: any) => {
    const data = rawData['dojo_starter-Game'];
    if (!data) return null;
...
const transformPiece = (rawData: any) => {
    const data = rawData['dojo_starter-Piece'];
    if (!data) return null;
...
const transformPlayer = (rawData: any) => {
    const data = rawData['dojo_starter-Player'];
    if (!data) return null;
...
const transformPlayerProfile = (rawData: any) => {
    const data = rawData['dojo_starter-PlayerProfile'];
    if (!data) return null;
...
export const useStarkDiceEntities = (pollInterval = 5000) => {
    ...
    const fetchAllEntities = async () => {
        try {
            ...
            const entityArray = Array.isArray(res) 
                ? res 
                : Object.values(res).flat();
            
          console.log("Raw dojo_starter entities:", entityArray);
            
            entityArray.forEach((entity) => {
                if (entity && entity.models) {
                    // Process DiceRoll
                    if ('dojo_starter-DiceRoll' in entity.models) {
                    ...
                    // Process Game
                    if ('dojo_starter-Game' in entity.models) {
                    ...
                    // Process Piece
                    if ('dojo_starter-Piece' in entity.models) {
                    ...
                    // Process Player
                    if ('dojo_starter-Player' in entity.models) {
                    ...
                    if ('dojo_starter-PlayerProfile' in entity.models) {
            ...
            console.error("Error fetching dojo_starter entities:", error);
        }
    };
