...
class Ludo {
  ...
  syncPositionsFromServer(piecesData: Piece[]) {
    ...
    for (const player of Object.keys(grouped) as Player[]) {
      grouped[player].forEach((pieceData, index) => {
        ...
        // UI.setPiecePosition(player, index, pieceData.position);
        console.log(
          "current fucking positions",
          this.currentPositions[player][index]
        );
        console.log("current server positions ", pieceData.position);
      });
    }
  ...
  constructor(actions: LudoActions) {
    ...
    this.resetGame();
    // UI.updatePiecesFromServer(this.actions.getPlayerPieces());
    // if (this.actions) {
    //   //   this.actions.getPlayerPieces().entries()
    //   this.syncPositionsFromServer(this.actions.getPlayerPieces());
    // }
  }
      ...
      ludoRef.current = new Ludo({
        ...
        getCurrentPlayerIndex: () =>
          gameStateRef.current.currentPlayer
            ? Number(gameStateRef.current.currentPlayer.index)
            : null,
        ...
        getCurrentTurnPlayer: () => {
          ...
          return Number(gameStateRef.current.game.current_turn) === 0
            ? "P1"
            : "P2";
        },
      });
